...
angular.module("myApp.controllers", []).controller("personCtrl", function($scope, Person) {
	...
	$scope.add = function() {
		...
		person.description = $scope.model.newPerson.description;
		person.title = $scope.model.newPerson.title;
		person.votes = 0;
		person.$save(function(response) {
			$scope.model.persons.push(response);
		});
		$scope.model.newPerson.title = '';
		$scope.model.newPerson.description = '';
	};
	
	$scope.remove = function(/** Idea  person, *//** Integer *//* index) {
		$scope.initiator = true;
		$scope.model.persons.splice(index, 1);
		person.$remove();
	};
	
	$scope.addVotes = function(*//** Idea *//* person, *//** Integer *//* votes) {
		$scope.initiator = true;
		person.votes += votes;
		person.$update();
	};
